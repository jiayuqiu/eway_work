...
class Port(object):
    ...
    def judge_pass_port(self, newest_weather_array, ship_port_line):
        ...
        if ('LNG' in ship_port_line[6]) | ('油' in ship_port_line[6]):
            ...
            # 判断吃水深度
            if ship_port_line[25]:
                if breth_port_bool:
                    if float(ship_port_line[25]) < tmp_breth_data.iloc[0, 6]:
    ...
    def get_port_data(self):
        ...
        select_sql = """
                     SELECT * from ship_port_data WHERE (berthing_control=0 OR depart_time>='%s' 
                     OR berthing_control IS NULL) AND (berth IS NOT NULL )
                     """ % today_date
    ...
    def update_port_data(self, breth_control, ship_port_data, ship_static_df):
        ...
        for index in range(ship_port_data_len):
            ...
            # print(ship_wind_resistance_level, breth_control[index][0], ship_port_data[index, 0])
            update_sql = """
                         UPDATE ship_port_data SET ship_type='%s', dwt='%f', wind_resistance_level='%d', 
                         berthing_control='%d', reason='%s' WHERE ipd_id='%d'
                         """ % (ship_type, ship_dwt, int(float(ship_wind_resistance_level)),
                                int(breth_control[index][0]), breth_control[index][1], int(ship_port_data[index, 0]))
            # print(update_sql)
