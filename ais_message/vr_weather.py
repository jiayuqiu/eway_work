...
class Weather(object):
    ...
    def report_mysql(self):
        ...
        return zhoushan_weather_status, max_avg_wind, max_zf_wind, min_njd
...
if __name__ == "__main__":
    ...
    while True:
        try:
            weather = Weather()
            weather_status_str, max_avg_wind_int, max_zf_wind_int, min_njd_int = weather.report_mysql()
            create_time = int(time.time())
            weather_out_str = str(create_time) + "," + weather_status_str + "," + str(max_avg_wind_int) + "," + \
                              str(max_zf_wind_int) + "," + str(min_njd_int) + "\n"
            create_date = time.strftime('%Y%m%d')
            weather_file = open('weather/weather_%s.csv' % create_date, 'a')
            weather_file.write(weather_out_str)
            time.sleep(1800)
        except Exception as e:
            print(e)
            time.sleep(60)
